...
class S3Backend(KeyValueStoreBackend):
    ...
    def __init__(self, **kwargs):
        ...
        self.endpoint_url = conf.get('s3_endpoint_url')
        self.aws_region = conf.get('s3_region')

        self.aws_access_key_id = conf.get('s3_access_key_id')
        self.aws_secret_access_key = conf.get('s3_secret_access_key')

        self.bucket_name = conf.get('s3_bucket')
        if not self.bucket_name:
            raise ImproperlyConfigured('Missing bucket name')

        self.base_path = conf.get('s3_base_path')
